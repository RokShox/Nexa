...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="getkeff", description="Parse MCNP output files for criticality results."
    )
    parser.add_argument("--head", action="store_true", help="Print header", default=False)
    parser.add_argument(
        "files", metavar="FILE", nargs="+", type=str, help="Path to the MCNP output files to parse"
    )
    ...
    with open("keff.txt", "w", encoding="utf-8") as o:
